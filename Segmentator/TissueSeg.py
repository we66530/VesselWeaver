...
# 1. Global Settings
# ============================================================
# Final TissueSeg outputs are always written here,
...
# The current TotalSegmentator tissue_4_types task does not support fast=True.
...
# ============================================================
# 3. Single-Case Tissue Segmentation
# ============================================================
def run_tissue_segmentation(
    ...
    """
    Run TotalSegmentator tissue_4_types for one CT only.
    ...
    if SKIP_IF_FINAL_EXISTS and final_nii.exists():
        print("=" * 100)
        print("[SKIP] Tissue segmentation atlas already exists.")
    ...
    print(f"==> Starting TotalSegmentator 'tissue_4_types' task on {device}...")
    ...
    if not final_nii.exists():
        raise FileNotFoundError(
            "TotalSegmentator finished, but the expected tissue atlas was not found:\n"
            f"{final_nii}"
        )

    file_size_mb = final_nii.stat().st_size / (1024 ** 2)
    print(f"==> Saved tissue NIfTI atlas: {final_nii}")
    print(f"==> File size: {file_size_mb:.2f} MB")
    print("==> Tissue label IDs follow TotalSegmentator tissue_4_types multilabel output.")
    print("\n" + "=" * 100)
    print("==> Single-case tissue segmentation completed successfully.")
    print("=" * 100)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run TotalSegmentator tissue_4_types segmentation for one CT NIfTI file."
    )
